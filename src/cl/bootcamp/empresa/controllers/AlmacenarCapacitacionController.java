 ...
 * @version 1.0.0
 * @see AlmacenarCapacitacionController
 * Creación de la clase AlmacenarCapacitacionController, contiene las
 * validaciones para Usuario y Capacitacion
 */
public class AlmacenarCapacitacionController {
    ...
    /**
     * Sobrescritura del método Capacitacion
     *
     * @return Capacitacion
     */
    public Capacitacion almacenarCapacitacionController() {
        ...
        System.out.println("(obligatorio) Ingresar RUT empresa con el formato " +
                "00000000-k: ");
        while (true) {
        ...
        System.out.println("(opcional) Ingresar día en este formato: lunes, " +
                "martes, miércoles, jueves, viernes, sábado o domingo");
        while (true) {
            capacitacion.setDia(sc.nextLine());
            if (capacitacion.getDia().isBlank()) break;
            if (!capacitacion.getDia().matches("lunes|martes|miércoles|jueves" +
        ...
        System.out.println("(opcional) Ingresar hora con este formato " +
                "HH:MM = 00:00");
        while (true) {
        ...
        while (true) {
            ...
            if (capacitacion.getLugar().length() < 10 ||
                    capacitacion.getLugar().length() > 50) {
                System.err.println("Error, ingresar lugar debe contener " +
                        "entre 10 a 50 caracteres");
            } else {
                break;
            }
        }
        ...
        while (true) {
            ...
            if (capacitacion.getDuracion().length() > 70) {
                System.err.println("Error, la duración debe ser menor a " +
                        "70 caracteres");
                continue;
            } else {
                break;
        ...
        while (true) {
            try {
                ...
                if (capacitacion.getCantidadAsistentes() > 1000) {
                    System.err.println("Error, la cantidad de asistentes no " +
                            "pueden pasar los 1000 asistentes");
                } else {
                    break;
                }
            } catch (InputMismatchException e) {
